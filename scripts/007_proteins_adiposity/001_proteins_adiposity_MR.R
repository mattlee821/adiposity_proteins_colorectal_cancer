...
exposure_data <- read.table("data/exposure_data/000_clumped_proteins.txt", header = T, sep = "\t")
...
write.table(exposure_data, "analysis/005_proteins_adiposity/exposure_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

## extract outcome data ====
filenames <- list.files(path = "/data/GWAS_data/files/pulit_2018_PMID30239722/processed/", pattern="txt", full.names=F)
filenames <- filenames[!grepl(paste0("adj", collapse = "|"), filenames)]
...
write.table(outcome_data, "analysis/005_proteins_adiposity/outcome_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

## harmonize data ====
harmonise_data <- harmonise_data(exposure_data, outcome_data, action = 2)
write.table(harmonise_data, "analysis/005_proteins_adiposity/harmonise_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

## MR ====
mr_results <- mr(harmonise_data, method_list = methods)
write.table(mr_results, "analysis/005_proteins_adiposity/mr_results.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

## Sensitivity analysis ====
mr_singlesnp <- mr_singlesnp(harmonise_data)
write.table(mr_singlesnp, "analysis/005_proteins_adiposity/mr_singlesnp.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

mr_hetrogeneity <- mr_heterogeneity(harmonise_data, method_list = methods_heterogeneity)
write.table(mr_hetrogeneity, "analysis/005_proteins_adiposity/mr_hetrogeneity.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

mr_pleiotropy <- mr_pleiotropy_test(harmonise_data)
write.table(mr_pleiotropy, "analysis/005_proteins_adiposity/mr_pleiotropy.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

mr_leaveoneout <- mr_leaveoneout(harmonise_data)
write.table(mr_leaveoneout, "analysis/005_proteins_adiposity/mr_leaveoneout.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
## Plots ====
...
### save plots ====
pdf("analysis/005_proteins_adiposity/figures/plot_mr_scatter.pdf")
for (i in 1:length(plot_mr_scatter)) {
  print(plot_mr_scatter[[i]])
}

pdf("analysis/005_proteins_adiposity/figures/plot_singlesnp_forest.pdf")
for (i in 1:length(plot_singlesnp_forest)) {
  print(plot_singlesnp_forest[[i]])
}
dev.off()

pdf("analysis/005_proteins_adiposity/figures/plot_leaveoneout_forest.pdf")
for (i in 1:length(plot_leaveoneout_forest)) {
  print(plot_leaveoneout_forest[[i]])
}
dev.off()

pdf("analysis/005_proteins_adiposity/figures/plot_mr_funnel.pdf")
