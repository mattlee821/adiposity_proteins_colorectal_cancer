...
harmonise_data_list <- fread("analysis/009_colocalisation/results/stratified_proximal/harmonise_data.txt")
...
for (i in 1426:length(harmonise_data_list)){
  
  label_exposure <- unique(harmonise_data_list[[i]]$exposure)
  label <- paste0(label_exposure, "_", "stratified_proximal")
  label_outcome <- "stratified_proximal"
  ...
  # save ====
  saveRDS(coloc_results, paste0("analysis/009_colocalisation/results/stratified_proximal/", label, ".RData"))
...
write.table(table_master, "analysis/009_colocalisation/results/stratified_proximal/001_coloc_results_missing.txt", 
