...
for (i in 1:length(exposure_list)){
  exposure_list[[i]]$id.exposure <- paste0(exposure_filenames[[i]], "_", "joint_distal_male")
}

# outcome data ====
filenames <- c("joint_distal_Male_wald_MAC50_1.TBL.annotated.txt")
...
for (i in 1:length(exposure_list)){
  ...
  outcome_list[[i]]$outcome <- "joint_distal_male"
...
write.table(exposure, "analysis/009_colocalisation/results/joint_distal_male/exposure_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
outcome <- bind_rows(outcome_list)
write.table(outcome, "analysis/009_colocalisation/results/joint_distal_male/outcome_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
...
write.table(harmonise_data, "analysis/009_colocalisation/results/joint_distal_male/harmonise_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
...
for (i in 1:length(harmonise_data_list)){
  ...
  label <- paste0(label_exposure, "_", "joint_distal_male")
  label_outcome <- "joint_distal_male"
  ...
  pdf(paste0("analysis/009_colocalisation/results/joint_distal_male/figures/", label, ".pdf"), 
      height = 10, width = 10)
  ...
  # save ====
  saveRDS(coloc_results, paste0("analysis/009_colocalisation/results/joint_distal_male/", label, ".RData"))
...
write.table(table_master, "analysis/009_colocalisation/results/joint_distal_male/001_coloc_results.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
