...
for (i in 1:length(exposure_list)){
  exposure_list[[i]]$id.exposure <- paste0(exposure_filenames[[i]], "_", "stratified_proximal")
}

# outcome data ====
filenames <- c("Stratified_proximal_Meta_125K_Results.tsv.annotated.txt")
...
for (i in 1:length(exposure_list)){
  ...
  outcome_list[[i]]$outcome <- "stratified_proximal"
...
write.table(table_master, "analysis/009_colocalisation/results/stratified_proximal/001_coloc_results.txt", 
