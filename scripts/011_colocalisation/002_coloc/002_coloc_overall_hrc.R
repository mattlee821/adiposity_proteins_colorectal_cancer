...
a <- subset(a, cancer == "overall-HRC-EUR")
...
for (i in 1:length(exposure_list)){
  exposure_list[[i]]$id.exposure <- paste0(exposure_filenames[[i]], "_", "overall_HRC")
}

# outcome data ====
filenames <- c("MarginalMeta_HRC_EUR_only_Results.tsv.annotated.txt")
...
for (i in 1:length(exposure_list)){
  ...
  outcome_list[[i]]$outcome <- "overall_HRC"
...
write.table(exposure, "analysis/009_colocalisation/results/overall_HRC/exposure_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
outcome <- bind_rows(outcome_list)
write.table(outcome, "analysis/009_colocalisation/results/overall_HRC/outcome_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
...
write.table(harmonise_data, "analysis/009_colocalisation/results/overall_HRC/harmonise_data.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
...
for (i in 1:length(harmonise_data_list)){
  ...
  label <- paste0(label_exposure, "_", "overall_HRC")
  label_outcome <- "overall_HRC"
  ...
  pdf(paste0("analysis/009_colocalisation/results/overall_HRC/figures/", label, ".pdf"), 
      height = 10, width = 10)
  ...
  # save ====
  saveRDS(coloc_results, paste0("analysis/009_colocalisation/results/overall_HRC/", label, ".RData"))
...
write.table(table_master, "analysis/009_colocalisation/results/overall_HRC/001_coloc_results.txt", 
            row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
